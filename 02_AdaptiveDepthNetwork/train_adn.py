...
def train_one_epoch_twobackward(
    ...
    for i, (image, target) in enumerate(metric_logger.log_every(data_loader, args.print_freq, header)):
        ...
        with torch.cuda.amp.autocast(enabled=scaler is not None):
            ...
            outputs_full = model(image, skip=skip_cfg_supernet)
            ...
            # default:forward pass for base_net
            outputs_skip = model(image, skip=skip_cfg_basenet)

            # Experimental #1 @ 2024.01.26: forward pass for random sub-network
            # skip_cfg_random_subnet = [bool(torch.randint(0,2,(1,)).item()) for _ in range(len(skip_cfg_basenet))]
            # if skip_cfg_random_subnet == skip_cfg_supernet: # remove super-net from random sub-network
            #     skip_cfg_random_subnet = skip_cfg_basenet
            # outputs_skip = model(image, skip=skip_cfg_random_subnet)
    
            #_, pred_skip = outputs_skip.topk(500, 1, largest=True, sorted=True)
...
def evaluate(model, criterion, data_loader, device, print_freq=100, log_suffix="", skip=None):
    model.eval()
    metric_logger = utils.MetricLogger(delimiter="  ")
...
def main(args):
    ...
    # print(f"models: {models.__dict__.keys()}")
    # sys.stdout.flush()

    # if args.model not in ("swin_t", "vit_b_16", "vit_b_32", "efficientnet_v2_s", "efficientnet_b2"):
    ...
    if args.test_only:
        ...
        if model_ema:
            evaluate(model_ema, criterion, data_loader_test, device=device, log_suffix="EMA", skip=skip_cfg)
        else:
            evaluate(model, criterion, data_loader_test, device=device, skip=skip_cfg)
        return
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_one_epoch_twobackward(model, criterion, criterion_kd, optimizer, data_loader, device, epoch, args, model_ema, scaler, skip_cfg_basenet, skip_cfg_supernet, subpath_alpha=args.subpath_alpha)

        lr_scheduler.step()

        if model_ema:
            evaluate(model_ema, criterion, data_loader_test, device=device, log_suffix="EMA", skip=skip_cfg_basenet)
            evaluate(model_ema, criterion, data_loader_test, device=device, log_suffix="EMA", skip=skip_cfg_supernet)
        else:
