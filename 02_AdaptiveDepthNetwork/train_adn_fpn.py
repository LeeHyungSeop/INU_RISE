...
def train_one_epoch(net, criterion, criterion_kd, optimizer, trainloader, device, alpha, epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        if (batch_idx % 100 == 0):
            ...
            print(f"Epoch[{epoch}][{progress * 100:.1f}%] loss:{loss:.3f}\tloss_full_acc:{loss_full_acc:.3f}\tloss_feature_kd:{loss_feature_kd:.3f}\tloss_softmax_kd:{loss_softmax_kd:.3f}")
            sys.stdout.flush()
...
def get_args_parser():
    ...
    parser.add_argument("--nproc_per_node", default=4, type=int, help="number of gpus per node")
    ...
    parser.add_argument('--output-dir', default='./checkpoint', help='path where to save')
...
def main(args):
    ...
    args.visible_device = os.environ.get("CUDA_VISIBLE_DEVICES") if os.environ.get("CUDA_VISIBLE_DEVICES") != None else '0'
    device='cuda'

    print(f"args: {args}")

    if args.model == 'ResNet50_ADN_FPN':
        net = ResNet50_ADN_FPN()
    elif args.model == 'MobileNetV2_ADN_FPN':
        net = MobileNetV2_ADN_FPN()
    else:
    ...
    if not args.test_only:
        if args.model == 'ResNet50_ADN_FPN':
            ...
            net = IntermediateLayerGetter(net, ['layer3_skippable', 'layer4_skippable', 'layer5_skippable', 'layer6_skippable']) 
...
'''
torchrun --nproc_per_node=4 train_adn_fpn.py --model ResNet50_ADN_FPN --batch-size 32 \
    ...
    --dataset-path /media/data/ILSVRC2012/ 2>&1 | tee ./logs/log_resnet50_adn_fpn.txt
'''