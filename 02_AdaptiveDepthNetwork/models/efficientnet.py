...
from .misc import Conv2dNormActivation, SqueezeExcitation, SkippableConv2dNormActivation
...
class MBConv(nn.Module):
    def __init__(
        ...
        layers.append(
            SkippableConv2dNormActivation(
                expanded_channels, cnf.out_channels, kernel_size=1, norm_layer=norm_layer, activation_layer=None, skippable=skippable,
            )
        )

        self.block = SkippableSequentialLayers(*layers)
        self.stochastic_depth = StochasticDepth(stochastic_depth_prob, "row")
        self.out_channels = cnf.out_channels

    def forward(self, input: Tensor, skip: bool=False) -> Tensor:
        result = self.block(input, skip)
...
class SkippableSequentialStages(nn.Sequential):
    ...
    def forward(self, input, skip: List[bool] = None):
        # The first and the last stages are not skippable in EfficientNet.
        # Hence, only accepts 'skip' flags for the skippable stages.
        assert len(skip) == (self.num_stages - 2), \
            f"The networks has {self.num_stages - 2} skippable stages, got: {len(skip)}"

        input = self[0](input, skip[0]) # non-skippable "pre" stage
        for i in range(1, len(self) - 1):
            input = self[i](input, skip[i-1])
        input = self[-1](input, skip[-1]) # non-skippable "post" stage
        return input
...
class EfficientNet(nn.Module):
    def __init__(
        ...
        stochastic_depth_prob: float = 0.1,  # woochul
        ...
        # Number of skippable stages. @woochul
        self.num_skippable_stages = len(inverted_residual_setting)
        ...
        layers.append(
            SkippableConv2dNormActivation(
                3, firstconv_output_channels, kernel_size=3, stride=2, norm_layer=norm_layer, activation_layer=nn.SiLU, skippable=False,
            )
        )
        ...
        for cnf in inverted_residual_setting:
            ...
            for i_layer in range(cnf.num_layers):
                ...
                skippable = False
                if (i_layer >= (cnf.num_layers // 2)):
                    skippable = True

                stage.append(block_cnf.block(block_cnf, sd_prob, norm_layer, skippable=skippable))
                stage_block_id += 1
        ...
        self.features = SkippableSequentialStages((len(inverted_residual_setting) + 2), *layers)
    ...
    def _forward_impl(self, x: Tensor, skip: List[bool]) -> Tensor:
    ...
    def forward(self, x: Tensor, skip: List) -> Tensor:
