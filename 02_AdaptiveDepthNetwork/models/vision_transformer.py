...
class EncoderBlock(nn.Module):
    ...
    def __init__(
        ...
        # self.ln_1 = norm_layer(seq_length)  # @woochul: for switchable-BN
        self.self_attention = nn.MultiheadAttention(hidden_dim, num_heads, dropout=attention_dropout, batch_first=True)
        self.dropout = nn.Dropout(dropout)

        # MLP block
        self.ln_2 = norm_layer(hidden_dim)
        # self.ln_2 = norm_layer(seq_length) # @woochul: for switchable-BN
        self.mlp = MLPBlock(hidden_dim, mlp_dim, dropout)

        # switchable Layer norm. -- wchkang
        if self.skippable == False:
            self.ln_1_skip = norm_layer(hidden_dim)
            self.ln_2_skip = norm_layer(hidden_dim)
            #  self.ln_1_skip = norm_layer(seq_length)  # @woochul: for switchable-BN
            #  self.ln_2_skip = norm_layer(seq_length)
            
            # @woochul: experiment for tesing no-swithable-layer-norm 
            # self.ln_1_skip = self.ln_1
            # self.ln_2_skip = self.ln_2

    def forward(self, input: torch.Tensor, skip = False):
        torch._assert(input.dim() == 3, f"Expected (batch_size, seq_length, hidden_dim) got {input.shape}")
        # Switchable norm. @woochul
...
class Encoder(nn.Module):
    ...
    def __init__(
        ...
        for i in range(num_layers):
            ...
            #     skippable = True
            # original 
            if ((i + 1) % num_blocks_per_stage == 0): # every last block of stages is skippable.
                skippable = True
            else:
                skippable = False
    ...
    def forward(self, input, skip: List[bool] = None):
        ...
        for i_block in range(len(self)):
            ...
            if self[i_block].skippable == True and skip[i_stage] == True:
                # print(f"-- skip {type(self[i_block])}")
                pass
            else:
                input = self[i_block](input, skip[i_stage])
                # print(f"-- execute {type(self[i_block])}")
        return input
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, skip: List[bool] = [False, False, False, False]):
